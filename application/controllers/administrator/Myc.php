...
//        $data['networks'] = $this->generateTree("banmabng");
    ...
    {
        $row1 = [];
        $row = $this->db->query('SELECT id_member, username from tbl_member WHERE id_upline="'.$parent_key.'"')->result_array();
        ...
        foreach($row as $key => $value)
        {
            ...
            $row1[$key]['pid'] = array_values($this->membersTree($value['id_member']));
        }
